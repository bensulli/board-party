...
function showTriviaModal(questionData) {
    ...
    if (questionData.question.imageUrl) {
        ...
        img.className = "mx-auto my-2 rounded-md max-h-48 object-contain";
...
function gameOverSequence() {
    ...
    if (startScreenH1)
        startScreenH1.textContent = "Congratulations! Play Again?";
    showScreen('start');
    resetFullGameState();
}
...
async function initializeApp() {
    ...
    if (questionsLoaded && playersList.length >= 2) {
        ...
        if (!existingErrorMsg) {
            let errorText = "";
            if (!questionsLoaded)
                errorText += "Could not load custom questions. ";
            if (playersList.length < 2)
                errorText += "Player config error. ";
