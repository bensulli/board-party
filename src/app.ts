...
let tokenPosition: number = 0; // 0 to NUM_SPOTS - 1
let visitedSpots: boolean[] = new Array(NUM_SPOTS).fill(false);
let questions: TriviaQuestionFormat[] = []; // This will be populated from JSON
let availableQuestions: TriviaQuestionFormat[] = []; // To draw from
...
const startButton = document.getElementById('start-button')! as HTMLButtonElement; // Get start button
...
function playIntroVideo() {
    ...
    videoPlayer.onerror = () => { // Handle video loading errors.
    ...
    if (INTRO_VIDEO_URL && (INTRO_VIDEO_URL.endsWith('.mp4') || INTRO_VIDEO_URL.endsWith('.webm') || INTRO_VIDEO_URL.endsWith('.ogv'))) { // Basic check for video file extensions
        ...
        videoPlayer.play().catch(e => { // Catch errors if play() is interrupted or fails (e.g., autoplay blocked).
...
function playCongratsVideo() {
    ...
    videoPlayer.onerror = () => { // Handle errors loading the congrats video.
    ...
    if (CONGRATS_VIDEO_URL && (CONGRATS_VIDEO_URL.endsWith('.mp4') || CONGRATS_VIDEO_URL.endsWith('.webm') || CONGRATS_VIDEO_URL.endsWith('.ogv'))) { // Basic check for video file extensions
        ...
        videoPlayer.classList.remove('hidden');
        videoPlaceholderText.classList.add('hidden');
        videoPlayer.play().catch(e => { // Catch play errors.
...
function gameOverSequence() {
    ...
    if(startScreenH1) startScreenH1.textContent = "Congratulations! Play Again?"; // Update start screen title.
